...
{
    if(key[SDL_SCANCODE_LEFT]) hero.theta -= 0.1f;
    if(key[SDL_SCANCODE_RIGHT]) hero.theta += 0.1f;
    return hero;
}
...
static Hero move(Hero hero, const char** const walling, const uint8_t* key)
{
      ...
      )
    {
        ...
        if(
            key[SDL_SCANCODE_W]
            || key[SDL_SCANCODE_UP]
          ) hero.velocity = add(hero.velocity, acceleration);
        if(
            key[SDL_SCANCODE_S]
            || key[SDL_SCANCODE_DOWN]
          ) hero.velocity = sub(hero.velocity, acceleration);
